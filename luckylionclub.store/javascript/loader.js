...
document.getElementById("favicon").href = `./images/png/logo.png`;
document.getElementById("price-img").src = `./images/gif/banner.gif`;

switch (collectionInfo.background.type) {
    case "video":
        document.getElementById("background-video").poster = `./images`;
        document.getElementById("background-video-source").src = `./images/png/background.png`;
        document.getElementById("background-video").load();
        break;
    case "image":
        document.getElementById("background-video").outerHTML = ''
        const imageStyle = `<style> .body { background-image: url('./images/png/background.png'); background-repeat: no-repeat; background-size: cover; background-attachment: fixed; background-position: center } </style>`
...
document.getElementById("lprice").innerText = `${mintInfo.price.toFixed(1)}`;
document.getElementById("ldate").innerText = `Whitelist Mint available ${collectionInfo.date}`;
...
document.getElementById("price").innerText = `${(mintInfo.price * mintInfo.minUnits).toFixed(1)}`
